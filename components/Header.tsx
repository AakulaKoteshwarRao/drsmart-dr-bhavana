...
const ALL_LINKS: Record<string, { href: string; label: string }> = {
  ...
  products:     { href: '/packages',            label: 'Packages' },
  testimonials: { href: '/reviews',        label: 'Testimonials' },
    ...
    <header className="header">
      <div className="header-inner">
        <Link href="/" className="logo" aria-label="Go to homepage">
          {clinic.logo ? (
            <Image src={clinic.logo} alt={clinic.name} width={200} height={80} style={{ height: 80, width: 'auto', maxWidth: 200, objectFit: 'contain', display: 'block' }} priority />
          ) : (
